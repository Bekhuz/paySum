...
const hammamaluch=document.querySelector('#hammamalumot'),
      login=document.querySelector('#profilelogin'),
      taxrirlash=document.querySelector('#taxrirlash1'),
      parol=document.querySelector('#profileparol')


    let shox = localStorage.getItem('login');
    let rux = localStorage.getItem('parol');

    login.hasAttribute(value,"shox")
    parol.innerHTML=rux;




    hammamaluch.addEventListener('cick', (event) => {
        event.preventDefault()
        a='';
        b='';
        c='';
